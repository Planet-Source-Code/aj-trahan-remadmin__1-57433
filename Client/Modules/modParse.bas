...
With objFileCollection
    For lLoop = 1 To .Count
        ...
        If Len(Trim(.Item(lLoop))) <> 0 Then
            ...
            ' **** LVFILES. (This will also prevent
            ' **** files with zero length to be listed and
            ' **** stop errors that would occur on file x-fer)
            If GGGG <> "(0 KB)" Then
                EEEE = EEEE & " " & GGGG
...
frmMain.lblRE(1).Caption = objFolderCollection.Count
