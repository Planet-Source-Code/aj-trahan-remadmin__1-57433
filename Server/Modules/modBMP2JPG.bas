...
Public Sub BMPtoJPG(Pic As PictureBox)
  Abort = False
  Dim i As Integer
  Dim filebmp
  Dim filejpg As String
  Dim c As New cDIBSection
  filebmp = App.Path & "\Desktop.bmp"
  Pic.Picture = LoadPicture(filebmp)
  filejpg = App.Path & "\Desktop.jpg"
  c.CreateFromPicture Pic.Picture
  SaveJPG c, filejpg, 75
